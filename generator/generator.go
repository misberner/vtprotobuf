...
type Extensions struct {
	...
	// SupportGenPrefix contains the Go package prefix for generated support functions for builtin types
	// in order to allow bootstrapping.
	// In normal vtproto mode, this will always be empty.
...
func NewGenerator(allFiles []*protogen.File, featureNames []string, ext *Extensions) (*Generator, error) {
	...
	targetGoPackages := make(map[protogen.GoImportPath]struct{})
	for _, f := range allFiles {
		if f.Generate {
			targetGoPackages[f.GoImportPath] = struct{}{}
		}
	}
	...
	return &Generator{
		...
		ext:              ext,
		features:         features,
		targetGoPackages: targetGoPackages,
	}, nil
}
...
func (gen *Generator) GenerateFile(gf *protogen.GeneratedFile, file *protogen.File) bool {
	p := &GeneratedFile{
		GeneratedFile:    gf,
		Ext:              gen.ext,
		TargetGoPackages: gen.targetGoPackages,
	}

	binaryName := "protoc-gen-go-vtproto"
	version := ""
	if bi, ok := debug.ReadBuildInfo(); ok {
		if bi.Path != "" {
			binaryName = path.Base(bi.Path)
		}
		version = bi.Main.Version
	}
	p.P("// Code generated by ", binaryName, ". DO NOT EDIT.")
	if version != "" {
		p.P("// ", binaryName, " version: ", version)
	}
