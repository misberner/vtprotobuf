...
func NewGenerator(allFiles []*protogen.File, featureNames []string, ext *Extensions) (*Generator, error) {
	...
	localGoPackages := make(map[protogen.GoImportPath]struct{})
	for _, f := range allFiles {
		if f.Generate {
			local[string(f.Desc.Package())] = true
			localGoPackages[f.GoImportPath] = struct{}{}
		}
	}
	...
	return &Generator{
		...
		features:        features,
		local:           local,
		localGoPackages: localGoPackages,
	}, nil
}
...
func (gen *Generator) GenerateFile(gf *protogen.GeneratedFile, file *protogen.File) bool {
	p := &GeneratedFile{
		...
		LocalGoPackages: gen.localGoPackages,
	}
